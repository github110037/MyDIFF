...
void connect_wire(axi4_ref &mmio_ptr, Vmycpu_top *top) {/*{{{*/
    // connect
    // mmio
    // aw   
...
void cemu_perf_diff(Vmycpu_top *top, axi4_ref &cpu_axi_ref) {/*{{{*/
    ...
    // cemu init{{{
    PaddrTop cemu_paddr_top;

    Pmem cemu_inst_mem = Pmem(inst_range);
    cemu_inst_mem.load_binary(0x1fc00000, "../nscscc-group/perf_test_v0.01/soft/perf_func/obj/allbench/inst_data.bin");
    cemu_paddr_top.add_dev(inst_range, &cemu_inst_mem);

    PaddrConfreg cemu_confreg = PaddrConfreg(false);
    cemu_paddr_top.add_dev(confreg_range, &cemu_confreg);
    // cemu }}}
    ...
    PaddrTop v_paddr_top;

    Pmem v_inst_mem = Pmem(inst_range);
    v_inst_mem.load_binary(0x1fc00000, "../nscscc-group/perf_test_v0.01/soft/perf_func/obj/allbench/inst_data.bin");
    v_paddr_top.add_dev(inst_range, &cemu_inst_mem);

    PaddrConfreg v_confreg = PaddrConfreg(false);
    v_paddr_top.add_dev(confreg_range, &cemu_confreg);
